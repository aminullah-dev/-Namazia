...
class MainViewModel @Inject constructor(
    ...
    fun loadPrayerTimes(date: LocalDate = LocalDate.now(af.namazia.app.utils.APP_ZONE)) {
            ...
            repository.getPrayerTimes(date, city, settings.calculationMethod, settings.asrSchool)
                .fold(
                    onSuccess = { entity ->
                        ...
                        if (date == LocalDate.now(af.namazia.app.utils.APP_ZONE)) {
                            AlarmScheduler.scheduleTodayAlarms(context, entity, settings)
                            af.namazia.app.ui.widget.AzanWidget.requestUpdate(context)
    ...
    fun loadMonthlyCalendar(
        year: Int = LocalDate.now(af.namazia.app.utils.APP_ZONE).year,
        month: Int = LocalDate.now(af.namazia.app.utils.APP_ZONE).monthValue
    ) {
        viewModelScope.launch {
    ...
    private fun PrayerTimesEntity.toPrayerTimeList(settings: AppSettings): List<PrayerTime> {
        val now = LocalTime.now(af.namazia.app.utils.APP_ZONE)
